...
import { WorkOrder, TAG_DESCRIPTIONS } from '../types';
...
export function NewWorkOrderForm({ onSubmit }: NewWorkOrderFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (err.message?.includes('work_orders_ot_key')) {
    ...
    <div className="bg-white p-6 rounded-lg shadow-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label htmlFor="tag" className="block text-sm font-medium text-gray-700 mb-1 flex justify-between">
              <span>TAG</span>
              <span className="text-gray-500 text-xs">
                {TAG_DESCRIPTIONS[formData.tag] || 'Seleccione un TAG'}
              </span>
            </label>
            <select
              id="tag"
        ...
        <div className="flex justify-end gap-3 mt-6">
          ...
          <button
            type="submit"
            className="px-4 py-2 bg-[#00843D] text-white rounded-md hover:bg-[#006e33]"
          >
            Crear OT
          </button>
        </div>
